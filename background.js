  ...
  if (message.type === 'GENERATE_TLDR') {
    handleTLDRRequest(message.tweetData, message.articleUrl, message.quotedTweetUrl)
      .then(async (result) => {
        ...
        var prefs = await chrome.storage.sync.get({
          autoDownloadMd: true,
          pushToLlmWiki: false,
        });
        ...
        if (prefs.pushToLlmWiki) {
          pushMarkdownToLlmWiki(message.tweetData, result.tldr, result.articleContent, result.quotedFullContent, result.isArticle, result.mode, senderTabId);
        }
...
// Last resort: save via chrome.downloads to the Downloads/x-learned/ subfolder.
...
async function writeViaDownloads(markdown, fileName) {
  var fullPath = 'x-learned/' + fileName;
...
function parseOwnerRepo(repo) {
  var trimmed = String(repo || '').trim().replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
  var parts = trimmed.split('/');
  if (parts.length < 2 || !parts[0] || !parts[1]) return null;
  return { owner: parts[0], repo: parts[1] };
}

function toWikiFileSlug(tweetData) {
  var handle = 'unknown';
  var statusId = '';
  var tweetUrl = (tweetData && (tweetData.tweetUrl || tweetData.url)) || '';
  try {
    var pathname = new URL(tweetUrl).pathname.split('/').filter(Boolean);
    if (pathname[0]) handle = pathname[0];
    var si = pathname.indexOf('status');
    if (si >= 0 && pathname[si + 1]) statusId = String(pathname[si + 1]).replace(/[^0-9]/g, '');
  } catch (_) { /* use defaults */ }
  var slug = (handle + (statusId ? '-' + statusId : ''))
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
  if (!slug) slug = 'bookmark';
  return slug + '.md';
}

function wikiPathWithAttempt(prefix, slugFile, attempt) {
  var base = String(prefix || 'raw/x-bookmarks').replace(/^\/+|\/+$/g, '');
  if (attempt <= 0) return base + '/' + slugFile;
  var stem = slugFile.replace(/\.md$/i, '');
  return base + '/' + stem + '-' + (attempt + 1) + '.md';
}
...
function wrapWikiFrontmatter(markdown, tweetData) {
  var title = (tweetData && tweetData.author) || 'X bookmark';
  title = String(title).replace(/"/g, "'");
  var source = (tweetData && (tweetData.tweetUrl || tweetData.url)) || '';
  var now = new Date();
...
async function pushMarkdownToLlmWiki(tweetData, tldr, articleContent, quotedFullContent, isArticle, mode, senderTabId) {
  try {
    ...
    if (!parsed) throw new Error('invalid llm wiki repo');

    var localData = await chrome.storage.local.get('encryptedGithubPat');
    if (!localData.encryptedGithubPat) throw new Error('missing GitHub token');
    var token = await decryptApiKey(localData.encryptedGithubPat);
    if (!token) throw new Error('missing GitHub token');

    var markdown = wrapWikiFrontmatter(
      buildMarkdownContent(tweetData, tldr, articleContent, quotedFullContent, isArticle, mode),
      tweetData
    );
    var slugFile = toWikiFileSlug(tweetData);
    var lastErr = 'push failed';
    for (var attempt = 0; attempt < 8; attempt++) {
      var path = wikiPathWithAttempt(syncData.llmWikiPath, slugFile, attempt);
      ...
      var res = await fetch(url, {
        ...
        body: JSON.stringify({
          message: 'ingest: x bookmark ' + slugFile.replace(/\.md$/i, ''),
          content: utf8ToBase64(markdown),
        }),
      });
      if (res.ok) {
        chrome.storage.local.set({
          lastWikiPush: { timestamp: Date.now(), success: true, path: path },
        });
        if (senderTabId) {
          chrome.tabs.sendMessage(senderTabId, {
            type: 'WIKI_PUSH_STATUS',
            success: true,
            path: path,
          }).catch(function () {});
        }
      ...
      if (res.status === 422 || res.status === 409) continue;
      var body = await res.text();
      lastErr = 'GitHub ' + res.status + (body ? ': ' + body.slice(0, 180) : '');
      break;
    }
    throw new Error(lastErr);
  } catch (err) {
    console.log('[background] llm wiki push error:', err.message);
    chrome.storage.local.set({
      lastWikiPush: { timestamp: Date.now(), success: false, error: err.message },
    });
    if (senderTabId) {
      chrome.tabs.sendMessage(senderTabId, {
        type: 'WIKI_PUSH_STATUS',
