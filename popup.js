...
async function migrateAndLoadSettings() {
  ...
  if (localData.encryptedGithubPat) {
    try {
      githubPatPlain = await decryptApiKey(localData.encryptedGithubPat);
    } catch (_) {}
  }

  document.getElementById('provider').value = syncData.provider;
  document.getElementById('apiKey').value = apiKeyPlain;
  document.getElementById('githubPat').value = githubPatPlain;
  ...
  document.getElementById('pushToLlmWiki').checked = !!syncData.pushToLlmWiki;
  document.getElementById('llmWikiRepo').value = syncData.llmWikiRepo || 'Young1108/llm_wiki';
  document.getElementById('llmWikiPath').value = syncData.llmWikiPath || 'raw/x-bookmarks';
...
function updateSavePathHint(available) {
  ...
  if (available) {
    hintEl.textContent = '未选择文件夹，保存到下载目录 x-learned 子文件夹';
  } else {
...
async function saveSettings() {
  try {
    ...
    var llmWikiRepo = document.getElementById('llmWikiRepo').value.trim() || 'Young1108/llm_wiki';
    var llmWikiPath = document.getElementById('llmWikiPath').value.trim() || 'raw/x-bookmarks';
    if (pushToLlmWiki && !githubPatPlain) {
      showStatus('推送到 llm_wiki 需要填写 GitHub Token（repo 权限）', 'error');
      return;
    }
    if (githubPatPlain) {
      var encPat = await encryptApiKey(githubPatPlain);
      await chrome.storage.local.set({ encryptedGithubPat: encPat });
    }
    ...
    await chrome.storage.sync.set({
      ...
      pushToLlmWiki: pushToLlmWiki,
      llmWikiRepo: llmWikiRepo,
...
async function loadDebugInfo() {
  ...
  try {
    ...
    var [syncData, localData, nativeResult] = await Promise.all([
      ...
      chrome.storage.local.get({ lastSave: null, lastWikiPush: null }),
    ...
    lines.push('Push llm_wiki: ' + (syncData.pushToLlmWiki ? 'on' : 'off') + ' (' + (syncData.llmWikiRepo || '') + ')');
...
var UPDATE_CHECK_REPO = 'Young1108/x-learned';
